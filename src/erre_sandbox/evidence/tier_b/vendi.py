...
The default kernel is ``"semantic"`` (MPNet,
...
A kernel-swap option parameterises ``_load_default_kernel`` to
accept an alternate HuggingFace encoder id. The MPNet instrument is
preserved as the default (``encoder_name=None``); the swap reports a versioned
amended metric ``vendi_semantic_v2_encoder_swap`` under the new encoder so
the original MPNet numbers stay reproducible. Pre-registered encoders for
the kant escalation are pinned in the encoder registry.
...
"""MPNet semantic cosine kernel (regression baseline instrument)."""

DEFAULT_ENCODER_MODEL_ID: str = _DEFAULT_ENCODER_MODEL_ID
"""Public alias of the default semantic encoder model id.

Exposed so downstream consumers (M10-0 individuation ``EmbeddingProvider``)
can single-source the production encoder id from this module instead of
...
def e5_passage_prefix() -> str:
    """Public accessor for the E5 ``passage:`` document-embedding prefix.

    The literal lives in :data:`_E5_PASSAGE_PREFIX`; this getter is the single
    sanctioned way for downstream consumers (the M11-C3b centroid panel's
    :func:`~erre_sandbox.evidence.individuation.layer1.build_embedding_provider`
    factory) to obtain it without re-importing the private name (MED-2).
    """
    return _E5_PASSAGE_PREFIX


def model_needs_e5_prefix(model_id: str) -> bool:
    """Whether ``model_id`` is an E5-family encoder that needs the prefix.

    The same membership test ``_load_default_kernel`` uses, exposed publicly so
    the centroid-panel factory threads the ``passage:`` prefix through the same
    rule (single source — M11-C3b §5.1 / MED-2).
    """
    ...
    *,
    ...
    Args:
        encoder_name: HuggingFace model id (semantic path only). ``None``
            falls back to MPNet (baseline, asserted by
            ``test_vendi_default_encoder_model_id_is_all_mpnet_base_v2``).
            the kernel-swap option swaps in a multilingual encoder
            (``intfloat/multilingual-e5-large`` or ``BAAI/bge-m3``) via this
            argument (D-2 pre-registered revision pins).
    ...
    needs_e5_prefix = model_needs_e5_prefix(model_id)
    prefix = e5_passage_prefix()

    def kernel(items: Sequence[str]) -> np.ndarray:
        if needs_e5_prefix:
            encode_inputs = [prefix + str(text) for text in items]
        else:
