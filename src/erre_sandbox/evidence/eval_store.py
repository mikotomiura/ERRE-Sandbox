...
* :func:`bootstrap_schema` — idempotent CREATE for ``raw_dialog.dialog``,
  ``metrics.tier_{a,b,c}`` and (M10-0, additive) ``metrics.individuation``.
  The raw column set is locked in lockstep with
  :data:`ALLOWED_RAW_DIALOG_KEYS` (module-load-time check); the
  individuation column order is owned by
  :mod:`erre_sandbox.evidence.individuation.ddl`.
* :func:`write_individuation_rows` — full-run-replace writer for
  ``metrics.individuation`` typed :class:`MetricResult` rows. On the
  metrics side of the contract, never a training egress.
...
from erre_sandbox.evidence.individuation.ddl import (
    TABLE_NAME as INDIVIDUATION_TABLE,
)
from erre_sandbox.evidence.individuation.ddl import (
    individuation_ddl_sql,
)
from erre_sandbox.evidence.individuation.ddl import (
    row_field_names as individuation_row_field_names,
)
...
class _DuckDBRawTrainingRelation:
    ...
    * Column projection is the **intersection** of the physical columns
    ...
    def __init__(self, conn: duckdb.DuckDBPyConnection) -> None:
        ...
        # Aggregate row-level contamination check:
        # ``connect_training_view()`` is the loader boundary
...
def bootstrap_schema(con: duckdb.DuckDBPyConnection) -> None:
    """Create raw_dialog + metric tier + individuation tables idempotently.
    ...
    *con* must be a writable connection (``read_only=False``); a
    ...
    # M10-0 individuation table (additive). The DDL is composed by the
    # individuation package from the bare table name; we qualify it with the
    # METRICS_SCHEMA constant here, never a hard-coded schema-dot literal, so
    # the CI eval-egress grep gate stays satisfied. Column order / typed-result
    # coupling live in erre_sandbox.evidence.individuation.ddl.
    con.execute(individuation_ddl_sql(METRICS_SCHEMA))


# ---------------------------------------------------------------------------
# M10-0 — individuation metric writer (metrics schema, NOT training egress)
# ---------------------------------------------------------------------------

_INDIVIDUATION_NATURAL_KEY: tuple[str, ...] = (
    "run_id",
    "individual_id",
    "metric_name",
    "channel",
    ...
    "tick",
    "source_filter_hash",
    # M11-C3b HIGH-2 (DA-M11C3b-P1-5): the multi-encoder centroid panel emits
    # one row per (dyad, encoder). source_filter_hash is window-derived and so
    # encoder-independent, so without embedding_model_id the 3 panel encoders'
    # rows collide on the natural key and dedup raises. Existing single-encoder
    # (MPNet) rows simply carry that id in the key — backward compatible.
    "embedding_model_id",
)


def write_individuation_rows(
    con: duckdb.DuckDBPyConnection,
    results: Sequence[MetricResult],
) -> int:
    """Persist individuation ``results`` into ``metrics.individuation``.

    **Semantics — full-run replace**: every ``run_id`` that
    appears in *results* has its existing ``metrics.individuation`` rows
    deleted before the batch is inserted. This is the intended contract for
    M10-0, where a run's full metric set is recomputed together; it is
    **not** safe for partial per-metric recompute (that would drop the
    run's other metrics). Callers must pass the complete result set for
    each ``run_id`` present.

    Raises :class:`ValueError` if *results* contains a duplicate natural key
    (``run_id, individual_id, metric_name, channel, aggregation_level, tick,
    source_filter_hash, embedding_model_id``) within the batch, so an in-batch
    double-write surfaces loudly rather than producing duplicate rows. The
    ``embedding_model_id`` component keeps the M11-C3b multi-encoder centroid
    panel's per-encoder rows distinct (HIGH-2). Returns the number of rows
    inserted.

    *con* must be writable. The schema is qualified from the
    :data:`METRICS_SCHEMA` constant (no ``metrics.`` literal).
    """
    if not results:
        return 0
    ...
    for result in results:
        key = (
            ...
            result.aggregation_level.value,
            result.tick,
            result.provenance.source_filter_hash,
            result.provenance.embedding_model_id,
        )
        if key in seen:
            msg = (
                "write_individuation_rows: duplicate natural key in batch:"
                f" {dict(zip(_INDIVIDUATION_NATURAL_KEY, key, strict=True))}"
            )
    ...
    con.execute(
        f"DELETE FROM {METRICS_SCHEMA}.{INDIVIDUATION_TABLE}"  # noqa: S608  # identifiers are module-private constants
        f" WHERE run_id IN ({placeholders})",
        run_ids,
    )

    column_list = ", ".join(individuation_row_field_names())
    value_placeholders = ", ".join("?" for _ in individuation_row_field_names())
    insert_sql = (
        f"INSERT INTO {METRICS_SCHEMA}.{INDIVIDUATION_TABLE}"  # noqa: S608  # identifiers are module-private constants
...
def export_raw_only_snapshot(
    ...
    single auditable egress route is a deliberate design choice.
    """
    src = str(src_path)
    out = str(out_path)
    if "'" in out:
