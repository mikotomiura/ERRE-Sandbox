...
sqlite ``dialog_turns`` table as JSONL.
"""
...
def _build_run_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "Escape hatch: explicit opt-in to the pre-SH-2 "
...
def _build_subcommand_parser() -> argparse.ArgumentParser:
    ...
    subparsers = parser.add_subparsers(
        ...
        required=True,
        metavar=(
            "{export-log,baseline-metrics,scaling-metrics,source-index,cache-benchmark}"
        ),
    )
    export_log.register(subparsers)
    baseline_metrics.register(subparsers)
    scaling_metrics.register(subparsers)
    source_index.register(subparsers)
    cache_benchmark.register(subparsers)
    return parser
...
def cli(argv: list[str] | None = None) -> int:  # noqa: PLR0911 — sub-command dispatch chain
    ...
    if effective_argv and effective_argv[0] in _SUBCOMMANDS:
        ...
        if args.subcommand == "source-index":
            return source_index.run(args)
        if args.subcommand == "cache-benchmark":
            return cache_benchmark.run(args)
