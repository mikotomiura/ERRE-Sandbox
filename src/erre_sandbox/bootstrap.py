...
class BootConfig:
    ...
    # ``allow_unauthenticated_lan`` escape hatch is the
    ...
    # M10-B individual layer toggle (default off). When enabled, the cognition
    # cycle distils a read-only SubjectiveWorldModel and injects its top-K into
    # the USER prompt, and bootstrap validates each persona projects to an
    # immutable PhilosopherBase at registration. Off → byte-identical to the
    # pre-M10-B prompt contract (DA-M10B-5). Frozen sibling of the per-row
    # DuckDB ``individual_layer_enabled`` training-egress gate.
    individual_layer: IndividualLayerConfig = field(
        default_factory=IndividualLayerConfig
    )
...
def _validate_ws_auth_config(cfg: BootConfig, *, token: str | None) -> None:
    ...
    Startup validation also requires that ``require_token=True`` without a
    ...
    * ``--allowed-origins=http://mac.local,http://g-gear.local`` (Origin gate;
      note: Godot's native WS client sends
    ...
    * ``--allow-unauthenticated-lan`` — explicit opt-in
      ...
      hide in production.
    """
    # require_token=True without a token is always a misconfig,
    ...
    if cfg.require_token and not token:
        msg = (
            "SH-2: --require-token=True but no token "
    ...
    msg = (
        ...
        " a loud warning on every startup)\n"
...
def make_agent_id(base_persona_id: str, ordinal: int = 1) -> str:
    """Derive an individual ``agent_id`` from a base persona id and an ordinal.

    Pure and deterministic. ``ordinal`` is 1-based and explicitly supplied by the
    caller (a multi-individual launcher passes its loop index) — there is **no
    hidden mutable counter**. ``ordinal=1`` reproduces
    the historical single-launch id byte-for-byte: ``make_agent_id("rikyu") ==
    "a_rikyu_001"``. Ordinals 1–999 zero-pad to three digits; larger values keep
    uniqueness but exceed the pad width.

    Raises ``ValueError`` for ``ordinal < 1`` (a 0-based or negative ordinal is a
    caller bug, never a silent ``_000``).
    """
    if ordinal < 1:
        msg = f"ordinal must be >= 1, got {ordinal}"
        raise ValueError(msg)
    return f"a_{base_persona_id}_{ordinal:03d}"
...
def _build_initial_state(
    ...
    *,
    ...
    ``agent_id`` is derived via :func:`make_agent_id` as
    ``a_<persona_id>_<ordinal:03d>``. ``ordinal`` defaults to ``1`` so a single
    launch is byte-identical to the historical ``a_<persona_id>_001``; a
    multi-individual launcher passes distinct ordinals to spawn several
    individuals of the same base (M11-C1, DA-M11C-2).
    ...
    agent_id = make_agent_id(spec.persona_id, ordinal)
...
async def bootstrap(cfg: BootConfig) -> None:  # noqa: PLR0915, C901 — composition root inherently long + branchy
    ...
    async with AsyncExitStack() as stack:
        ...
        cycle = CognitionCycle(
            ...
            reflector=reflector,
            individual_layer=cfg.individual_layer,
        )
        ...
        for spec in cfg.agents:
            ...
            if cfg.individual_layer.enabled:
                # M10-B (DA-M10B-5): boot-time invariant — every registered
                # agent's persona must project cleanly into an immutable
                # PhilosopherBase. This exercises the base-layer path from live
                # YAML (a projection failure fails boot loudly) without holding
                # per-agent state on the shared cycle (multi-agent crosstalk) or
                # touching the SYSTEM prompt. Persisting an IndividualProfile is
                # deferred to M10-C.
                base = PhilosopherBase.from_persona_spec(persona)
                logger.info(
                    "[bootstrap] individual layer ON: PhilosopherBase projection "
                    "validated for %s (%d cognitive habits)",
                    base.persona_id,
                    len(base.cognitive_habits),
                )
            logger.info(
