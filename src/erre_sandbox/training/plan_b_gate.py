...
Promoted from a standalone corpus-stats audit script into the
...
def audit_corpus(
    ...
    *,
    ...
    module-level constants. The standalone audit wrapper and the
    ``train_kant_lora`` ``--plan-b-gate`` path BOTH bind the production
    constants here; neither exposes the kwargs as CLI flags
    (preregistered gates must not have an operator-facing way to move
    thresholds post-hoc).
    """
