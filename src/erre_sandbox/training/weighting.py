...
This module is the **training-time** half of the retrain v2 spec (§3.2/§3.3).
...
* per-example raw weights via :func:`compute_example_weight` (formula
  clamped to ``[0.1, 3.0]``);
...
* a pre-training audit JSON via :func:`emit_weight_audit` that surfaces
  ...
  signals that drive the Candidate C fallback trigger.

Coefficient transparency: the 0.35 / 0.20 / 0.15 / 0.30
...
"""Lower bound on the raw per-example weight.
...
"""Upper bound on the raw per-example weight.
...
than the formula's nominal ~30x.
"""
...
_LANG_FACTORS: Final[dict[str, float]] = {
    ...
    "en": 1.0,
    "mixed": 0.5,
    "ja": 0.05,  # β rebalance: 0.2 → 0.05 で ja silent sink 抑制
}
...
diluted training example. Japanese was originally down-weighted to 0.2x
(56.7% ja-heavy corpus、non-discriminative ja conversation pattern 抑制)、
but the v4 weight-audit (kant_r8_v4) showed ja still consumed **38.9%** of
weighted mass even after the 0.2x factor (raw count 53.8% → weighted
38.9%、−15pt のみ)。verdict eval shard には ja が不足 (n_ja=21/18 = single
100-utterance window 取れず) なため、38.9% の weighted gradient が
"verdict 不可視の silent gradient sink" となり、rank=8 で de + ja の
capacity 競合を起こしていた (H4 dominant 仮説)。

β rebalance で **ja factor を 0.05 に大幅 down-weight** (採用案 A1)。
線形寄与は ``_COEFF_LANG × 0.05 = 0.35 × 0.05 = 0.0175``。

**補足**: 当初は「ja example の
大半が raw 段階で WEIGHT_CLAMP_MIN=0.1 floor に clamp される」と記述
したが、これは実際の math と矛盾する (HIGH-2 verbatim、`test_weighting.py:
69-93` の `test_compute_example_weight_ja_short_dialog_zero_marker` で
ja short/dialog/zero-marker 実値が 0.2875 > 0.1 で floor に届かないことを
明示)。線形寄与 0.0175 は最小だが、length / monolog_bonus / marker_factor
が下支えして合計 raw weight は ~0.28-0.40 程度になり、normalize 後の ja
weight も非自明な値を持つ。

**正確な見込み**: A1 (`_LANG_FACTORS["ja"]=0.05`) 単独では ja_mass を
v4 baseline 0.389 から下げるが、v5 hard floor (`JA_MASS_MAX_V5=0.10`) を
達成できる保証は **math 上は無い** (実 corpus の length / marker 分布
依存)。v5 hard gate (``plan_b_v5_gate.audit_corpus_v5``) は corpus
audit 出力に対する empirical hard floor として機能し、未達なら
``--plan-b-gate-v5`` exit 9 で retrain を中断する。**v5 retrain GO 判定は
本 weighting 変更だけでなく、必ず dry-run audit (`--dry-run --plan-b-gate
--plan-b-gate-v5`) で実 corpus の ja_mass を観測して PASS 確認することが
mandatory**。不足する場合は別 PR で A2 (corpus collect-time ja drop) や
A6 (hybrid) への pivot が必要。
"""
...
Heuristic, NOT empirical. Language gets the largest share
...
def compute_example_weight(example_metadata: dict[str, object]) -> float:
    ...
    Notes:
        ...
        will drift relative to the K-β rank=8 baseline.
    """
...
def normalise_weights_to_mean_one(raw: Sequence[float]) -> list[float]:
    """Rescale ``raw`` so the mean is exactly 1.0.
...
def emit_weight_audit(
    ...
    * ``per_language_weighted_mass["de"] + ["en"] < 0.60`` → soft warning only
    ...
    Args:
        ...
        extra: Optional mapping of caller-supplied provenance fields to
            merge into the audit dict *before* JSON write. Used by
            :func:`erre_sandbox.training.train_kant_lora._pre_training_audit`
            to surface the hybrid provenance block so
            ``weight-audit.json`` carries forensic-replay information
            required by the Gate 1 provenance binding. Caller keys
            overwrite standard fields on collision (defensive merge is
            caller's responsibility — standard field names like
            ``n_examples`` / ``n_eff`` are stable and not expected to
            collide with provenance keys).

    Returns:
        The audit dict (same content as the JSON file, including any
    ...
    # Merge caller-supplied
    # provenance fields (e.g. the ``da18_hybrid`` block) before write so
    # the JSON file and the returned dict stay in sync.
    if extra:
        audit.update(extra)
...
# The batch-level reduce was changed from ``sum/weights.sum()`` to ``.mean()``
# so that per-example weights survive the batch=1 + grad_accum gradient path
# under the VRAM-saturated Qwen3-8B + NF4 + rank=8 retrain configuration.
# ---------------------------------------------------------------------------


def compute_weighted_causal_lm_loss(logits: Any, labels: Any, weights: Any) -> Any:
    """Compute per-example weighted causal-LM loss (internal next-token shift).
    ...
    4. Reduce across the batch as ``(per_example_loss * weights).mean()``
       (``.mean()`` reduce, replaces the prior
       ``.sum() / weights.sum()`` form so that weight magnitude is
       preserved in the gradient under ``per_device_train_batch_size=1``).

    **Semantic note (replaces the original ``sum/weights.sum()`` reduce)**:

    Under ``per_device_train_batch_size=1`` (the VRAM-saturated
