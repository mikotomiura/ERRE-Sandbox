...
def test_build_initial_state_threads_ordinal_for_same_base() -> None:
    """A multi-individual launcher gets distinct ids by passing distinct ordinals."""
    spec = AgentSpec(persona_id="kant", initial_zone=Zone.STUDY)
    persona = _load_kant_for_tests()
    ids = [_build_initial_state(spec, persona, ordinal=n).agent_id for n in (1, 2, 3)]
    assert ids == ["a_kant_001", "a_kant_002", "a_kant_003"]
...
def test_make_agent_id_byte_exact_for_ordinal_one() -> None:
    """Single-launch default reproduces the historical id byte-for-byte."""
    assert make_agent_id("rikyu", 1) == "a_rikyu_001"
    assert make_agent_id("rikyu") == "a_rikyu_001"  # default ordinal


def test_make_agent_id_pads_to_three_digits() -> None:
    assert make_agent_id("kant", 2) == "a_kant_002"
    assert make_agent_id("kant", 42) == "a_kant_042"
    assert make_agent_id("kant", 999) == "a_kant_999"


def test_make_agent_id_above_pad_width_stays_unique() -> None:
    """Ordinals > 999 exceed the zero-pad width but remain distinct."""
    assert make_agent_id("kant", 1000) == "a_kant_1000"
    assert make_agent_id("kant", 1000) != make_agent_id("kant", 999)


@pytest.mark.parametrize("bad", [0, -1, -999])
def test_make_agent_id_rejects_ordinal_below_one(bad: int) -> None:
    with pytest.raises(ValueError, match="ordinal must be >= 1"):
        make_agent_id("kant", bad)
