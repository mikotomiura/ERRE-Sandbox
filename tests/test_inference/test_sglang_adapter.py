...
async def test_chat_round_trip_parses_openai_shape() -> None:
    ...
    # PR-14 DPN14-1.1: NO_LORA_SENTINEL omits lora_path so the request hits
    # the base model.
    assert "lora_path" not in body
...
async def test_chat_with_unknown_adapter_raises_before_network() -> None:
    ...
    assert chat_calls == []
    # PR-14 DPN14-1.1: error message points the caller at the sentinel as the
    # explicit base-model opt-out.
    # (The match above guards substring presence; this assertion is informative
    # only and lives in the per-lang test file for a stricter assert.)
