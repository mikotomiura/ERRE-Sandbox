"""凍結 manifest (prereg v2 §8、Codex HIGH-1 / HIGH-2).
...
import pytest
from scripts import skill_lever_manifest as man
...
def test_consistency_detects_code_changed_after_certification(tmp_path: Path) -> None:
    ...
    scorer = root / "scripts" / "skill_lever_scorer.py"
    scorer.write_text(
...
def test_consistency_detects_power_choice_mismatch(tmp_path: Path) -> None:
    ...
    power.write_text(json.dumps({**obj, "chosen": {"R": 8, "K": 10}}), encoding="utf-8")
    got = man.consistency_violations(root)
    assert "power table: chosen (R, K) differs from the scorer's frozen values" in got


@pytest.mark.skipif(
    not (_ROOT / man.MANIFEST).exists(), reason="manifest not written yet (pre-freeze)"
)
def test_committed_artifacts_are_consistent() -> None:
    """凍結後: commit 済みの manifest・certification・power 表が現在と一致する."""
