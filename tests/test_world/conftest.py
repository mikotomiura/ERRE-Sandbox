...
class MockCycleCall:
    ...
    tick_seconds: float
    # M10-C: the carried individual-layer world-model state passed by the
    # runtime. ``object`` keeps the fixture free of a cognition dependency;
    # ``None`` on every flag-off run (the default in these world tests).
