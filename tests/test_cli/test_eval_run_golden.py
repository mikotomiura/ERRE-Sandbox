...
async def test_capture_stimulus_writes_focal_turns(tmp_path: Path) -> None:
    ...
    try:
        ...
        # PR-Z (Codex HIGH-2 + user 条件 #2): stimulus paths never wire the
        # ``individual_layer_enabled`` kwarg, so every row must materialise as
        # the DDL DEFAULT ``FALSE`` (semantic/logical row invariance excluding
        # ``created_at``). ``IS NOT FALSE`` is stronger than ``= TRUE`` because
        # it also fails on any NULL — pinning the column's bivalent contract.
        non_false = con.execute(
            "SELECT COUNT(*) FROM raw_dialog.dialog"
            " WHERE individual_layer_enabled IS NOT FALSE"
        ).fetchone()
        assert non_false is not None
        assert non_false[0] == 0, (
            "PR-Z stimulus default-False contract violated:"
            f" {non_false[0]} row(s) had individual_layer_enabled != FALSE"
        )
    finally:
        con.close()
