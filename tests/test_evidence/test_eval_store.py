...
def test_bootstrap_creates_metric_tables(tmp_path: Path) -> None:
    # M10-0: bootstrap is now additive — the three tiers plus the M10-0
    # individuation table (DB schema is an additive change; CLI/output
    # behavior is unchanged). See .steering/.../decisions.md DA-M10I-9 (H5).
    db = tmp_path / "fresh.duckdb"
    con = _writable(db)
    try:
        bootstrap_schema(con)
