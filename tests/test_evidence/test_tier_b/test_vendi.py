...
# --- M11-C3b: e5 prefix public helpers (MED-2) -----------------------------


def test_e5_passage_prefix_is_passage_literal() -> None:
    assert e5_passage_prefix() == "passage: "


def test_model_needs_e5_prefix_true_for_e5_family() -> None:
    assert model_needs_e5_prefix("intfloat/multilingual-e5-large") is True
    assert model_needs_e5_prefix("intfloat/E5-base") is True  # case-insensitive


def test_model_needs_e5_prefix_false_for_non_e5() -> None:
    # MPNet (default) + BGE-M3 are not E5 family -> no passage prefix.
    assert model_needs_e5_prefix(DEFAULT_ENCODER_MODEL_ID) is False
    assert model_needs_e5_prefix("BAAI/bge-m3") is False
