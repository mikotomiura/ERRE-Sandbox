...
**Before any of that, it prints a public-safe environment block.** The first
lines of the output start with `=== environment (public-safe) ===`, so a
single pasted log carries both the environment a report was run under and the
result below it — no extra step, no free-text "OS:" field to fill in by hand.
The block is generated by `scripts/report_environment.py` from a fixed
allowlist: OS family/release/version and CPU architecture, the C runtime
...
diagnostic only, never part of the verdict, which stays the exit code and the
`=== summary ===` / `[hashes]` lines. Run
`python scripts/report_environment.py` on its own to print just this block:
it needs neither `uv` nor anything beyond the standard library, so even a
machine where `uv sync` itself failed can still produce a usable report.
...
## 7. How reports are counted

These rules were frozen on 2026-09-26 — before a single external report had
been received — precisely so a result already seen could not bend them.

**Outcomes.** A report's outcome is one of four, matching the issue
template's dropdown:

| outcome | meaning |
|---|---|
| `PASS` | every target ran (no `--only`), exit code 0, and `[verify-committed-artifacts] ALL OK` |
| `FAIL` | every target ran, exit code non-zero |
| `PARTIAL` | ran with `--only`, so the `[hashes]` line is `SKIP` — that table covers every target and cannot be checked for a subset. **`PARTIAL` is not counted as a `PASS`** |
| `NOT_RUN` | could not get far enough to run the verifier at all (clone, install, or `uv sync` failed) |

**Every report is listed.** A report that comes in is added to the tally
whatever its outcome; `NOT_RUN` and `FAIL` are not filtered out. The only
reports excluded are ones that are spam, unrelated to this repository, or
whose commit cannot be identified — and the number excluded for that reason
is stated alongside the count, never silently dropped.

**A `FAIL` is triaged, not erased.** Every failing report is sorted into
exactly one of:

- *claim-narrowing* — it followed the documented procedure and still failed,
  so the claim's scope is narrower than stated and that narrowing is written
  into the record;
- *documented-precondition-violated* — it departed from a precondition this
  file states (for example, re-checking out with a different
  `core.autocrlf`);
- *project-bug* — the verifier or the instructions were themselves at fault.

A `project-bug` gets fixed, but the report that found it **stays `FAIL`** in
the tally — fixing the bug does not retroactively turn a past failure into a
pass.
...
way). It may never be summarised as "reproduces anywhere" or
"reproduces on any machine": only the counted cells.

**The 2026-11-30 window.** Reports arriving after that date are still added
to the table, but whether a given report counts toward the paper's own tally
is fixed by a snapshot taken at that date, stated alongside the number in the
text. A report arriving later is recorded and dated, not used to reopen that
snapshot's count.
