...
- [x] G-GEAR 側で `feat/m8-baseline-quality-metric` が main に merge 済
      を確認、`git pull origin main` で `f5d5e7f` を取得
- [x] `nvidia-smi` で VRAM 13GB 以上空きを確認
- [x] `ollama list` で `qwen3:8b` + `nomic-embed-text` が存在
- [x] `uv run erre-sandbox --help` と `uv run erre-sandbox export-log --help`
...
- [x] orchestrator をブート → stdout/log に例外なく起動ログ
- [x] 別 shell で:
  ```bash
  uv run python -c "
  import asyncio
...
- [x] 同様に FSM 遷移の不正パスで `ValueError` 発生を Python REPL で確認

## Run 1 — bias_p=0.1 (60-90s)

- [x] G-GEAR で orchestrator を起動:
  ```bash
  ERRE_ZONE_BIAS_P=0.1 uv run erre-sandbox \
    --db var/run-01-bias01.db \
    --personas kant,nietzsche,rikyu
  ```
- [x] 別 shell で `uv run python evidence/_stream_probe_m6.py
  --out /tmp/run-01.jsonl --duration 80`
- [x] 同時に Godot client 側で目視観察、スクショ 3 枚:
  - top-down hotkey `0` の 100m 全景
  - zone 拡大 (chashitsu / agora / garden のいずれか)
  - ReasoningPanel (MIND_PEEK 開いた状態)
- [x] 80s 経過で Ctrl-C で orchestrator 停止
- [x] 成果物を `.steering/20260425-m7-beta-live-acceptance/run-01-bias01/` に保存:
...
- [x] `uv run erre-sandbox export-log \
    --db var/run-01-bias01.db \
...
- [x] 同手順を `ERRE_ZONE_BIAS_P=0.2`、DB は `var/run-02-bias02.db`、
      保存先は `run-02-bias02/`
- [x] Post-run の export-log / baseline-metrics も同様に

## Analysis (30-45 min)

- [x] 2 run の `summary.json` から agent ごとの zone 滞留 tick % を抽出
- [x] β 受け入れ条件 6 項目 (Rikyū/Kant/Nietzsche 分布 + 目視 3 項目) を評価
- [x] #88 受け入れ条件 2 項目 (export-log 完走 + persona filter 数)
- [x] #89 受け入れ条件 4 項目 (baseline JSON shape + 3 metric 値)
- [x] `bias.fired` 発火頻度比較 (run1 vs run2)、`bias_fired_rate` との整合
...
- [x] bias_p production default の判断 (0.1 / 0.2 / 他)
- [x] 気になった不具合 → hotfix PR 起票 or γ に送る

### `baseline.md` (#88 + #89 の 6 項目)

- [x] 2 run の baseline JSON を table 化:
...
- [x] 平均 / 分散 / 代表値を記録
- [x] CSDG 単著閾値 (0.30 / 0.50) を参照値として並記
- [x] M9 LoRA 比較 run 時に diff する reference として「凍結」宣言

## Follow-up

- [x] `observation.md` の結論を
      `.steering/20260424-m7-differentiation-observability/decisions.md`
      に D9 として追記
- [x] `baseline.md` の凍結宣言を
      `.steering/20260424-steering-scaling-lora/decisions.md` の L6 D1 に
      「baseline 固定済 (n=2)、M9 比較準備完了」として追記
- [x] memory `project_m7_beta_merged.md` を更新
      (acceptance 完了 → 次タスクは `m8-scaling-bottleneck-profiling`
      または Slice γ)
- [x] 必要なら `ERRE_ZONE_BIAS_P` default 変更 hotfix PR
- [x] γ 着手時に `observation.md` + `baseline.md` を Read (Plan 材料に)
