...
## GAP-1: WorldRuntime → Gateway の実配線スクリプトが未実装 ✅ 解消 (T21 / 2026-04-20)

> **解消記録**: PR #36 (T21 `feature/t21-m2-functional-closure`) で
> `src/erre_sandbox/bootstrap.py` + `src/erre_sandbox/__main__.py` + `pyproject.toml` の
> `[project.scripts] erre-sandbox` entry point として composition root を導入。
> さらに live 検証中に発見した 2 件の bug:
>   (a) `bootstrap.py` で `MemoryStore.create_schema()` 未呼出 → 初回 tick で `no such table: episodic_memory` 例外
>   (b) `cognition/cycle._build_envelopes` の MoveMsg target が「現在 x/y/z + zone 差し替え」→ `step_kinematics` の `locate_zone(dest.x,dest.y,dest.z)` で現在 zone を再計算 → zone 跨ぎ不能 → observation が pending に積まれず episodic_memory が populate されない
> を修正 ((a) は `bootstrap.py` で `memory.create_schema()` 追加、(b) は `world/tick.py:_consume_result` で `locate_zone(tgt) ≠ tgt.zone` を検知し `default_spawn(tgt.zone)` に resolve)。
>
> MASTER-PLAN §4.4 の 4 検収項目のうち #1-#3 を実機 evidence 付きで PASS、#4 は Mac 側 Godot 録画で視覚確認。Evidence: `.steering/20260419-m2-functional-closure/acceptance-evidence.md`。
...
|---|---|---|---|---|
| GAP-1 | WorldRuntime↔Gateway 配線欠落 | Avatar live 不可 | **T21 `m2-functional-closure` で解消** | ✅ 解消 (T21 / 2026-04-20, PR #36 + bug fix 2 件) |
