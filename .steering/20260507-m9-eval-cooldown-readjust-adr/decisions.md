...
## Lock-step update 2026-05-08 — Phase A run1 calibration 完了 (5/5 cells)

G-GEAR Phase A run1 calibration が **kant only × 5 wall (120/240/360/480/600)
sequential** で完了 (2026-05-08 18:49)。Mac 受領 + md5 10/10 一致 + DuckDB
read_only sanity 5/5 OK で確定値を `.steering/20260430-m9-eval-system/
decisions.md` ME-9 **Amendment 2026-05-08** ブロックに **lock-step で追記**
(本タスクの Amendment 2026-05-07 と整合):
...
- wall-aligned contention factor = 1.502 (run102 360 min single ÷ run0 360 min
  3-parallel)、§B.3 default 1.76× より **−14.7%**
- run2-4 wall_budget 確定 = **600 min** (parallel rate 1.057 /min × 0.85
  efficiency margin → wall_budget_raw 557 → rounded 600)
- 旧 D-1 (run102/103/104 採取) は **解消済**、blockers.md で move

本タスクの ADR (Amendment 2026-05-07) と上記 amendment は **rate basis 明示 +
context-aware trigger zone + wall-aligned 校正** で一貫、Codex 9th review
verdict (hybrid A/C、cooldown 再調整不要) は **5 cell 実測値で confirmation
済** (run100/101 の 1.625/1.596 /min が saturation curve 上の点として
妥当だったことが run102-104 で確認できた)。

### W-2 (memory growth signal) 解消方向

5 cell 実測 (1.625 → 1.596 → 1.592 → 1.571 → 1.552) が単調減少で 1-step diff
が逐次小さくなる典型的な saturation pattern を示し、cell-to-cell variance
ではなく **wall-duration accumulation** が dominant signal と判明。Codex M3
の懸念 (memory pressure signal の検出力) は 5 点 trend で empirically resolved。

### W-3 (drain grace 60s 適合性) 確認

run104 (wall=600 min single) の sidecar で `drain_completed=true` /
`runtime_drain_timeout=false` を確認 (Mac DuckDB sanity 経路で全 5 cell が
status=partial / stop_reason=wall_timeout / drain_completed=true)。drain
grace 60s は wall=600 min × single でも sufficient と empirical 確定、Codex
L1 の懸念は resolved。
