...
| 反復 frozen-context bank | repetitive frozen-context bank | B SECONDARY entropy lever の実体。enriched substrate から live 器官が 1 pass で生成した凍結 (prompt, sampling) を、chat() へ M 回投入して 1-sample を M-sample marginal に変換する construction 装置。**H/MDE/divergence 非計算 = measurement でない**。凍結 context を K 個で bank | B impl-design ADR §I0、bank.py、[[bake-out]] |
| bake-out | bake-out (retrieve-count=0) | 凍結 (prompt, sampling) を full cycle を通さず chat() へ直投入する M-loop 方式。retriever/store/evidence を一切呼ばない (retrieve-count=0 は構造的) = 最強の continuity-gate。SPDM-channel 直接接続を型的に封鎖する | B impl-design ADR §I1.4/§I2、test_bank_mloop_retrieve_count_zero |
| provenance pass | substrate provenance pass | 凍結 context を hand-constructed 文字列でなく enriched substrate から canonical builder が render した実 prompt にする 1 full-cycle pass。T3 materiality + live-channel-conformance を担保。この 1 pass のみ EclDecisionRecord を生む (M-loop は BankLlmCallRecord) | B impl-design ADR §I1.3/§I3.4、DA-BIMPL-2 |
| pre-bias readout | pre-bias readout | M-loop の readout を `_bias_target_zone` (cycle.py:1643, ERRE_ZONE_BIAS_P) 適用前の parsed `LLMPlan.destination_zone` に固定する規約。lever の対称 preferred_zones と post-LLM bias の交絡を殺し「変動源 = qwen3 sampling のみ」を成立させる前件 | B impl-design ADR §I1.4、Codex 事実誤認 HIGH-1、DA-BIMPL-8 |
