...
|---|---|---|---|
| 2026-05-09 | K-α #1: SGLang install Windows incompat | **NOT vLLM fire** (CS-1 amendment、Linux 境界に boundary 引き直し) | K-α report (本 runbook §5) |
| 2026-05-13 | K-β 実訓練 + real adapter confirmation | **NOT FIRED** | adapter swap ~8ms (60x margin)、bench overhead 2.5% (CS-7 4 trigger NON-FIRE)、CS-6 PEFT direct load 成功 (HTTP 200)、chat round trip 成功。SGLang-first 確定、vLLM defer 継続 (D-1)。本 runbook §4 / §5、CS-7 + CS-8 amendment 2026-05-13 |
