...
### 5.10 建設 pivot の第一歩 — construction milestone-2 = **M2（N体 embodied society）確定**（2026-07-11、doc-only）

計測 pivot（§5.9 の disposition B）を受けて、次に建てる construction milestone を **M2（N体 embodied society）**に
確定した（reimagine α[M2-first]×β[M4-first] → **M2 spine + β可視化graft + M3 post**、Codex Adopt-with-changes）。
- **なぜ M2 か**: 名は "3D **Society**"、Vision は「文化・儀式が立ち上がる生きた村」＝ inter-agent 現象。単一器官
  （ECL v1 GO）は成熟済ゆえ、次の不可欠な建設は**器官を N 体へ束ねて並行に動かす**こと。
- **二層**: Layer1（N体 composition ＋ 並行 determinism）が milestone を定義する GATING。Layer2 =
  **ミラー・シム（mirror-sim）**（他者観察→共有潜在→自己変調の self-other functional analog、守破離「守＝模倣」
  直結、`glossary.md`）は**有界の建設試行**（難航しても M2 を潰さない）。ミラー・シムは 4 規律付き
  （construction-mode only／functional analog 語彙＝✗ミラーニューロン実装／予算ゼロ SimToM prompt／
  appraisal measurement 非混入）。
- **honest な非対称**: 「N体化は計測 teeth には非最適（B-nbody で defer 裁定）」だが、**計測ラインが閉じた今**、
  N体を後回しにしていた理由（計測非最適）は選定を支配しなくなり、常に北極星だった建設理由（society＝名/vision）が
  支配する。cherry-pick でなく、CLOSE という事実が計算を変えた。
- **不可侵**: これは**建設であって計測でない**（floor/verdict/scorer を出さない、acceptance は causal wiring/
  boolean のみ）。arc measurement-line CLOSE は不変、holding 不可侵、reasoning-trace door 保全のまま。
- **次**: M2 の impl-design ADR（HOW）→ 実コード（Loop Engineering）、いずれも別タスク。

ファイル: ADR local `.steering/20260711-m13-post-cproper-disposition/` + `.steering/20260711-m13-construction-m2-scoping/`
（design-final / design-v1/v2/comparison / codex-review.md verbatim / decisions.md）、memory
`project_m13_post_cproper_disposition` / `project_m13_construction_m2_scoping`、`research-positioning §8/§9`。
...
- **多体化**（B。集団/組織レベルの発散、Project Sid [4] 系）＝ **construction milestone-2 = M2 として確定済**
  （§5.10、2026-07-11）。核候補次元 = self-other/mirror functional analog（守破離「守」）。
