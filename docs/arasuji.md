...
- **二層**: Layer1（N体 composition ＋ 並行 determinism）が milestone を定義する GATING。Layer2（**self-other/
  mirror** ＝他者観察→共有潜在→自己変調、守破離「守＝模倣」直結）は**有界の建設試行**（難航しても M2 を潰さない）。
  mirror は 4 規律付き（construction-mode only／functional analog 語彙／予算ゼロ SimToM prompt／appraisal
  measurement 非混入）。
