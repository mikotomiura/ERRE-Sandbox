...
- **推論 (実)**: Ollama 上の `qwen3:8b` (GGUF Q5_K_M, ~5.2GB)。SGLang は M9-B 計画で SGLang-first 採用 (multi-LoRA は SGLang v0.3+ でサポート)
...
- WebSocket 認証は **shared-token (opt-in) + Origin allow-list + session cap=8** の 3 層独立 (LAN 内前提)。`require_token=False` がデフォルトのため Mac↔G-GEAR LAN rsync workflow は無影響。`host=0.0.0.0` で全ゲート無効の組合せは `bootstrap()` が startup `RuntimeError` で誤公開を予防 (SH-2)
...
| Milestone | Scope (MVP) | Status |
|---|---|---|
| M10-0 | 個体化 metric + dataset manifest + cache benchmark + prompt ordering contract | MERGED |
| M10-A | 二層 schema scaffold (feature flag default off、persona YAML rename しない) | MERGED |
| M10-B | read-only SubjectiveWorldModel synthesis + USER prompt 注入 (LLMPlan 未変更) | MERGED |
| M10-C | bounded `WorldModelUpdateHint` 投入 + `cited_memory_ids` verify | MERGED |
| M11-A | NarrativeArc + coherence_score (diagnostic only) | MERGED |
| M11-B | DevelopmentState S1→S2→S3 transition (Python indirect signal) | MERGED |
| M11-C1 | id contracts + `WorldModelSnapshot` + `IndividualProfile` (snapshot pattern、DA-M11C1-4/-5、live home は AgentRuntime 維持) | MERGED |
| M11-C2 | `individual_state_trace` flag-on conditional sink + `belief_variance` (real flag-on は M10-A evaluation epoch + promotion sink 前提) | MERGED |
| M11-C3a | ja Burrows parity (`tokenise_ja` tier_a public 化 + `preprocessed_tokens` 注入) | MERGED |
| M11-C3b | rikyu (ja) for feasibility pilot に限定 (DA-M11C-8 supersede)、kant は補助 control に格下げ。chain (C3b ADR + C3b-exec + PR-Z) は bounded conclusive REJECT で terminate 済 (PR #294 MERGED 2026-05-29、reactivate path (a)〜(e) は別 ADR 経由のみ) | TERMINATED |
| M12+ gate | S4/S5、retirement、rebirth、multi-base society、individual LoRA | gated by the M10-11 framework close ADR (PR #297) |
