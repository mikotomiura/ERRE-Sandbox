...
5. **issue ごとに subagent で実行する**。task を main セッションで直実装せず、各 issue (I) を
   専任 subagent に委譲する (下記「モデル割当」)。main セッションはオーケストレータに徹し、
   issue 実装・レビューは subagent が担う。

## モデル割当 (per-issue subagent の担当・binding)

**各 issue (I) は subagent で実行し、役割ごとにモデルを固定する** (CLAUDE.md「モデル選択」表を
Loop の per-issue フローに具体化したもの)。main セッション直実装は Loop の意図ではない
(オーケストレータに徹する)。

| 役割 | 担当 subagent / model | 単位 | 権限 |
|---|---|---|---|
| **実装 (issue 実装しきる)** | `/loop-issue` executor = **Sonnet** subagent (実装/テスト/リファクタ) | issue 毎 | 実装 (worktree write) |
| **レビュー (issue コードレビュー)** | `code-reviewer` = **Opus** subagent | issue 毎 (必要時) + TASK-POST | read-only |
| 検証 (毎 attempt の客観確認) | `test-runner` / `loop-watchdog` = **Haiku** | 毎 attempt | 監視・停止のみ |
| タスク境界レビュー (統合 diff) | `code-reviewer`(Opus) ∥ Codex(gpt-5.5) | task 毎 1 回 | read-only |

- **実装系 = Sonnet**: `/loop-issue` は Sonnet subagent で起動する (大規模変更 = 10 ファイル以上は
  Sonnet[1m])。リネーム/フォーマットのみの issue は Haiku 可。
- **レビュー = Opus**: issue の高難度判断・コードレビューは Opus (`code-reviewer`) が担う。設計判断は
  Plan mode (Opus) が上流で確定済み前提。
- **検証 = Haiku**: 毎 attempt の exit-code 客観検証と意味判断は安価な Haiku (`test-runner` /
  `loop-watchdog`)。高頻度ゆえコスト最小化。
- コスト根拠: マルチエージェントは ~15x トークンを消費しうるため、Opus レビューは issue 毎 (必要時) と
  task 境界に絞り、毎 attempt の高頻度検証は Haiku に固定する。
- **subagent spawn は user 明示承認**が要る (`feedback_subagent_delegation_explicit_only`)。Loop 起票時に
  実装系 (Sonnet) / レビュー (Opus) の spawn 方針を承認しておく。
