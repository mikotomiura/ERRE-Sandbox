...
## CI cross-platform float drift 対応 (PR #55 fail → 修正)
PR #55 が CI (Linux) で golden checksum test fail。**timeout でなく cross-platform libm float drift** (Windows で
bake、Linux で verify、`math.cos/sin` が UCRT vs glibc で 1-ULP 相違、γ の RNG run-sequence が露呈)。
- user 方針裁定 = **B 量子化** (A=Linux bake は golden を Linux 固有化し handoff の cross-machine 目的に反する)。
- WSL Ubuntu-22.04 (=CI Linux と checksum 一致) で drift 実測 = max 8.88e-16 (1 ULP)。
- 修正 (ec26619) = emitted float + checksum を 6 桁量子化。**盲点 = envelope_provenance の embedded JSON 文字列内
  float** も封鎖 (subagent が WSL 実測で発見)。WSL 量子化 bake と Windows 量子化 bake が全 4 artifact byte 一致を
  実測 = CI 往復なしで緑保証。詳細 memory = `feedback_golden_crossplatform_float_drift`。
...
- **局所 pre-push 緑は cross-platform を保証しない**。golden/checksum の byte 契約は WSL Linux で cross-platform
  byte 一致を実測して初めて CI 緑を保証できる。cross-machine handoff は本質的に cross-platform ゆえ量子化必須。
